...
$(document).ready(function(){
 ...
 {
  $.ajax({
   url:"/livetable/fetch_data/<?php echo e($group->id_groups); ?>",
   dataType:"json",
  }).done(function(data)
   
   {
    ...
    for(var i=1; i<=15; i++){
    html += '<td contenteditable class="column_name" data-column_name="date_z'+[i]+'" data-id="'+data[count].id_Student+'">'+data[count].date_z+[i]+'</td>';
    html += '<td contenteditable class="column_name" data-column_name="nr_cw_z'+[i]+'" data-id="'+data[count].id_Student+'">'+data[count].nr_cw_z+[i]+'</td>';
